...
class ProductsImagesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
			...
			$prodstring = $this->ProductsImages->Products->get($this->request->data['product_id']);
			$ItemCodePhase = $prodstring->item_code;
			
			$ColorId = $this->request->data['color_id'];
			$colorstring = $this->ProductsImages->Colors->get($ColorId);
				
			$ColorPhase = substr(strtoupper($colorstring->name), 0,2);
			$ColorPhase = $ColorPhase.$ColorId; 
		
			//Final Product Code
			$this->request->data['product_code'] = 	$ItemCodePhase.$ColorPhase;

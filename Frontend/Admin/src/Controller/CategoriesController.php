...
class CategoriesController extends AppController
{
    ...
    public function index()
    {
        $this->paginate = [
			...
			'conditions'=>['Categories.id NOT IN'=>1]
        ];
  ...
  /*   public function view($id = null)
    ...
    {	
		...
		$pareCategory = $this->Categories->find('all')
		->where(['parent_id IS' =>NULL])
		->toArray();
		
		
		$categ=$this->Categories->find('all')
		->contain('ChildCategories')
		->where(['parent_id IS' =>$pareCategory[0]->id])
		->toArray(); 
		
		$AllCategories=array();
		$AllCategories[1] = '';
		foreach($categ as $subcat){

					$AllCategories[$subcat['id']] = $subcat['name'];
		}
       ...
       /*  $parentCategories = $this->Categories->ParentCategories->find('treeList'); */
        $this->set(compact('category', 'AllCategories'));
        $this->set('_serialize', ['category']);
    }
